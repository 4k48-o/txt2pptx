"""
PPT Service Files API - PPT 服务文件管理路由
"""
...
router = APIRouter(prefix="/ppt/files", tags=["ppt"])
...
@router.post(
    ...
    summary="上传参考文件（PPT 服务）",
    description="上传文件到 Manus，返回 file_id 用于创建 PPT 任务时附加",
)
async def upload_file(
    ...
    logger.info(f"Uploading file for PPT service: {file.filename} ({file_size} bytes)")
